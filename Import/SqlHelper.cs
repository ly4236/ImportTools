...
namespace Import
{
    ...
    public abstract class SqlHelper
    {
        ...
        {
            SqlConnection conn = new SqlConnection("Data Source=.;Initial Catalog=erecord;Integrated Security=True");
            string sql = string.Format("SELECT TOP 1 * FROM {0}", tablename);
            SqlCommand command = new SqlCommand();
            PrepareCommand(command, conn, null, sql, null);
            SqlDataReader reader = command.ExecuteReader(CommandBehavior.CloseConnection);

            string[] tablenames = new string[reader.FieldCount];

            for (int i = 0; i < reader.FieldCount; i++)
            {
                tablenames[i] = reader.GetName(i);
            }
        ...
        public static bool AddModels(DataTable dt, Dictionary<string, string> sss, out string sErrorMessage)
        {
            bool bResult = true;
            sErrorMessage = "";


            using (SqlConnection conn = new SqlConnection("Data Source=.;Initial Catalog=erecord;Integrated Security=True"))
            {
                conn.Open();
                using (SqlTransaction trans = conn.BeginTransaction())
                {
                    try
                    {
                        using (SqlBulkCopy bulk = new SqlBulkCopy(conn, SqlBulkCopyOptions.Default, trans))
                        {
                            ...
                            bulk.DestinationTableName = "caijishang3";
